...
| Path | What it is |
|---|---|
| `Web_T4C_GestCom/` | **The active app** — Blazor Server, .NET 8, EF Core, SQL Server. Has its own [CLAUDE.md](Web_T4C_GestCom/CLAUDE.md) — read that before making any code change, it documents build/test commands, layered architecture, document numbering, and stock/payment logic in detail. |
| `Web_T4C_GestCom.Tests/` | xUnit + bUnit + Moq test project for the app above. Run via `dotnet test` from inside `Web_T4C_GestCom/` (see that project's CLAUDE.md for exact commands). |
| `Web_T4C_GestCom/deploy/prod/` | Docker/VPS deployment for the live instance at `http://gestioncom.tijaraflow.fr/`. See `DEPLOY.md` there; follows the shared OVH VPS conventions from the user's global instructions (nginx-proxy + acme-companion already running, networks declared `external: true`, `$http_upgrade` not `$connection_upgrade` in vhost config). |
...
| `EnterpriseArchPlan.md`, `PROJECT_CODE_ANALYSIS_SUMMARY.md`, `PlanPourDev.md`, `Plan_Test_Manuel_T4C_GestCom.docx` | Planning/analysis documents at various points in the project's history. May be stale — verify against actual code in `Web_T4C_GestCom/` before relying on them. |
| `.github/agents/app-pattern-analyst.agent.md` | A read-only subagent definition scoped to `Web_T4C_GestCom` for tracing architecture/data-flow patterns before implementing a change in that style. |

## Where to actually work

Almost all code changes happen inside `Web_T4C_GestCom/`. `cd` there and follow
[Web_T4C_GestCom/CLAUDE.md](Web_T4C_GestCom/CLAUDE.md) — it has the build/run/test commands,
...
When touching anything under `Web_T4C_GestCom/deploy/`, follow the shared OVH VPS Docker rules
