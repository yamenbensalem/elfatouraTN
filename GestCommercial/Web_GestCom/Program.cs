...
// EF Core, and every service shared with GestCom_Desktop's AppHost.cs — see
// AddGestComServices's own doc comment for what's shared vs. host-specific.
builder.Services.AddGestComServices(builder.Configuration.GetConnectionString("DefaultConnection"));
...
// Host-specific current-user implementation; everything else came from AddGestComServices above.
...
using (var scope = app.Services.CreateScope())
{
    ...
    // Retenue à la source (Web_GestCom/TODO.md, section "Retenues à la source").
    db.Database.ExecuteSqlRaw("""
        IF NOT EXISTS (
            SELECT * FROM INFORMATION_SCHEMA.COLUMNS
