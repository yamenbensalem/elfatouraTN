...
.DESCRIPTION
    Web_GestCom/Program.cs initialise sa base à chaque démarrage via EnsureCreated() suivi
    ...
    ressemble une base GestCom correctement initialisée" — ce script ne la réimplémente PAS
    (ce serait fragile : un bloc oublié ou mal ordonné livrerait un schéma cassé à un client). Il se
    contente de :
      1. Créer une base vide et jetable ($TemplateDatabaseName, jamais GestCom — voir le garde-
         fou ci-dessous) sur l'instance SQL Server donnée par -Server.
      2. Lancer l'exécutable déjà buildé de Web_GestCom avec la variable d'environnement
         ...
         standard, donc AUCUN fichier sous Web_GestCom/ n'est lu pour la chaîne de connexion ni
         modifié. Vérifié empiriquement pendant le développement de ce script (override pointé vers
         un host bogus => l'appli a bien tenté de s'y connecter, pas vers YAMEN/GestCom).
...
.PARAMETER Server
    Instance SQL Server cible. Par défaut celle utilisée par Web_GestCom en Development
...
.PARAMETER TemplateDatabaseName
    ...
    "GestCom" (la vraie base de dev) — le script refuse de démarrer si c'est le cas, puisqu'il
    DROP/CREATE cette base à chaque exécution.

.PARAMETER TimeoutSeconds
    Délai maximum d'attente du démarrage complet de Web_GestCom (EnsureCreated + ~50 migrations
...
.EXAMPLE
    ...
    Produit deploy/db-template/GestCom_Template.bak sur l'instance YAMEN.
...
param(
    [string]$Server = "YAMEN",
    [string]$TemplateDatabaseName = "GestCom_Template",
...
if ($TemplateDatabaseName -eq "GestCom") {
    throw "TemplateDatabaseName ne peut pas être 'GestCom' (la vraie base de dev/test) — " +
          "ce script DROP puis CREATE cette base à chaque exécution, ce serait destructeur. " +
          "Utilise le nom par défaut (GestCom_Template) ou un autre nom clairement 'jetable'."
}
...
if (-not (Test-Path $webCsproj)) {
    throw "Web_GestCom.csproj introuvable à '$webCsproj' — vérifie que ce script est toujours " +
          "sous GestCom_Desktop/deploy/sql/ dans le repo."
}
...
Write-Host "GestCom_Template — préparation de l'image de base modèle" -ForegroundColor Cyan
...
# ── [2/5] Build de Web_GestCom ───────────────────────────────────────────
Write-Host "==> [2/5] Build de Web_GestCom ($Configuration)" -ForegroundColor Cyan
dotnet build $webCsproj -c $Configuration --nologo -v quiet
if ($LASTEXITCODE -ne 0) { throw "dotnet build de Web_GestCom a échoué." }

$exeCandidates = Get-ChildItem -Path (Join-Path $webProjectDir "bin\$Configuration") -Filter "Web_GestCom.exe" -Recurse -ErrorAction SilentlyContinue
if (-not $exeCandidates) {
    throw "Web_GestCom.exe introuvable sous bin\$Configuration\ après le build. Le build a peut-être échoué silencieusement."
}
...
Write-Host "==> [3/5] Lancement de Web_GestCom contre '$TemplateDatabaseName' (init DB uniquement, pas de requête HTTP)" -ForegroundColor Cyan
...
try {
    ...
    while ($elapsed -lt $TimeoutSeconds) {
        ...
        if ($proc.HasExited) {
            ...
            throw "Web_GestCom s'est arrêté prématurément (code $($proc.ExitCode)) pendant l'initialisation de '$TemplateDatabaseName'. Erreur :`n$stderrTail"
        }
    ...
    if ($proc -and -not $proc.HasExited) {
        Write-Host "    Arrêt du processus Web_GestCom (PID $($proc.Id))..." -ForegroundColor Gray
...
Write-Host ""
Write-Host "Rappel : relance ce script à chaque changement du schéma de Web_GestCom" -ForegroundColor Yellow
