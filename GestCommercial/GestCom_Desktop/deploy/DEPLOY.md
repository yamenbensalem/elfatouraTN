# Déploiement client — GestCom Desktop
...
1. build-client-package.ps1
   (régénère d'abord Database/GestCom_Template.bak
...
| Obfuscar | `dotnet tool install --global obfuscar.globaltool` | Obfusque `GestCom_Desktop.dll` avant livraison |

## Outils à installer chez le client (avant le premier lancement)

| Outil | Pourquoi | Remarque |
|---|---|---|
| **.NET 8 Desktop Runtime (x64)** | (https://dotnet.microsoft.com/en-us/download/dotnet/8.0 -- windows x64)Fait tourner `GestCom_Desktop.exe` et `GestCom_LicenseGenerator.exe` — le build est framework-dependent, pas autonome | Runtime seul, pas le SDK. S'il manque, Windows affiche lui-même un lien de téléchargement Microsoft au lancement — pas besoin de le pré-télécharger |
...
- **`App/`** — l'application, en Release, sans PDB, avec `GestCom_Desktop.dll` obfusqué
  (renommage des membres privés/champs — voir section 1 pour le détail et les limites : la
  logique métier dans `Web_GestCom.Core.dll` n'est volontairement **pas** obfusquée).
- **`LicenseTool/`** — `GestCom_LicenseGenerator.exe`, utilisé par le client (`collect`, sans
  risque) puis par toi (`issue`, avec ta clé privée qui ne quitte jamais ta machine).
- **`Database/`** — `GestCom_Template.bak` (image initiale : schéma + données de référence +
...
lance une fois `Web_GestCom` contre une base vide — son propre `Program.cs` fait tout le travail
...
à chaque changement de schéma dans `Web_GestCom` (sinon le `.bak` dérive silencieusement).

## 0. Prérequis (une seule fois sur la machine de Yamen)

- `dotnet tool install --global obfuscar.globaltool` (voir section 1).
- La clé privée existe à `GestCom_Desktop/deploy/keys/gestcom-license-private.pem`. Si ce fichier
...
Si le schéma de `Web_GestCom` a changé depuis la dernière livraison, régénère d'abord l'image de
base initiale (sinon `build-client-package.ps1` refuse de continuer si elle n'existe pas encore) :

```powershell
cd GestCom_Desktop/deploy
./sql/prepare-template-database.ps1
# -> deploy/db-template/GestCom_Template.bak
```
...
2. Obfusque **uniquement** `GestCom_Desktop.dll` avec Obfuscar (voir le commentaire en tête de
   `deploy/Obfuscar.xml` pour le détail complet, notamment **pourquoi
   `Web_GestCom.Core.dll` (entités + services) n'est PAS obfusqué** — deux essais réels ont
...
Avant de livrer : lancer `App\GestCom_Desktop.exe` (dans le package produit) contre une vraie
...
```
cd LicenseTool
GestCom_LicenseGenerator.exe collect
```
...
```powershell
cd GestCom_Desktop/deploy
.\client-package\Nom-Du-Client\LicenseTool\GestCom_LicenseGenerator.exe issue `
    --fingerprint "chemin\vers\fingerprint.json" `
    --client "Nom Du Client SARL" `
    --key "keys\gestcom-license-private.pem" `
...
1. **Recommandé** : `%ProgramData%\GestCom\license.lic`
   (généralement `C:\ProgramData\GestCom\license.lic` — créer le dossier `GestCom` s'il
   n'existe pas).
2. **Dépannage rapide** : à côté de `GestCom_Desktop.exe`, sous le nom `license.lic`. Pratique
...
Avant de créer le compte applicatif (ci-dessous), la base `GestCom` doit exister. Le package
client contient dans `Database/` un `GestCom_Template.bak` — produit sur la machine de Yamen par
...
résout le dossier data/log par défaut de l'instance cible, et refuse d'écraser une base `GestCom`
...
```
net user svc_gestcom "<mot de passe fort généré>" /add
```
...
Ça crée un login Windows dédié, scoped à `db_datareader` + `db_datawriter` sur `GestCom`
...
Faire tourner `GestCom_Desktop.exe` sous ce compte (tâche planifiée configurée "Exécuter en
tant que" `svc_gestcom`, ou raccourci `runas /user:.\svc_gestcom`). La chaîne de connexion
...
4. Ajuster `GestCom_Desktop/appsettings.json` : remplacer `Server=<NomMachine>` par
...
retrouver le mot de passe du compte `svc_gestcom`, désactiver le pare-feu local, ou réactiver
...
Lancer `App\GestCom_Desktop.exe`. Si la licence est valide, l'écran de connexion s'affiche
...
```
%ProgramData%\GestCom\logs\app-<date>.log
```
...
`%LOCALAPPDATA%\GestCom\logs\` — regarder les deux emplacements si le premier est vide. Un
...
`GestCom_Desktop/deploy/keys/gestcom-license-private.pem` est **l'artefact le plus critique de
