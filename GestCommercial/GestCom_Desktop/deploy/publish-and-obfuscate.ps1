...
<#
.SYNOPSIS
    Builds, obfuscates and packages GestCom_Desktop for handoff to a client.

.DESCRIPTION
    1. dotnet publish (Release, framework-dependent, no PDBs — see the csproj Release DebugType=none).
    2. Runs Obfuscar (deploy/Obfuscar.xml) against GestCom_Desktop.dll only — see that file's
       header comment for exactly why Web_GestCom.Core.dll is deliberately left unobfuscated.
...
.EXAMPLE
    ...
    Then manually smoke-test deploy/dist/GestCom_Desktop.exe against a real database before
...
$desktopCsproj = Join-Path $repoRoot "GestCom_Desktop\GestCom_Desktop.csproj"
...
Write-Host "==> Obfuscating GestCom_Desktop.dll" -ForegroundColor Cyan
...
Copy-Item (Join-Path $obfDir "GestCom_Desktop.dll") $OutputDir -Force
...
Write-Host ""
Write-Host "MANDATORY before handoff: run $OutputDir\GestCom_Desktop.exe against a real database," -ForegroundColor Yellow
