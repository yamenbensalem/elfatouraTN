...
.DESCRIPTION
    ...
    2. Publishes GestCom_LicenseGenerator (Release, framework-dependent) into a LicenseTool/
       subfolder — the client runs `GestCom_LicenseGenerator.exe collect` from there, and Yamen
    ...
    3. Copies deploy/sql/*.sql and deploy/sql/restore-database.ps1 into a Database/ subfolder, plus
       deploy/db-template/GestCom_Template.bak — the initial database image produced by
...
$licenseToolCsproj = Join-Path $repoRoot "GestCom_LicenseGenerator\GestCom_LicenseGenerator.csproj"
$sqlDir            = Join-Path $PSScriptRoot "sql"
$keysDir           = Join-Path $PSScriptRoot "keys"
$dbTemplateBak     = Join-Path $PSScriptRoot "db-template\GestCom_Template.bak"
...
if (-not (Test-Path $dbTemplateBak)) {
    throw "Image de base introuvable : $dbTemplateBak`n" +
          ...
          "à partir du schéma actuel de Web_GestCom — ce script refuse de livrer un package client " +
...
Write-Host "  LicenseTool/  — GestCom_LicenseGenerator.exe (collect côté client, issue côté Yamen)" -ForegroundColor Green
Write-Host "  Database/     — GestCom_Template.bak + restore-database.ps1 + create-app-login.sql" -ForegroundColor Green
Write-Host ""
