...
namespace GestCom_Desktop.Forms.CommandesAchat;
...
public class CommandesAchatListForm : Form
{
    ...
    public CommandesAchatListForm()
    {
        Text = "Commandes Achat";
        Width = 940;
        Height = 600;
        ...
        _grid.Columns.Add("EtatCommandeAchat", "État");
        _grid.Columns.Add("EtatReception", "Réception");
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            Logger.DebugLoadingList("commandes achat");
            commandes = await commandeService.GetAllAsync();
            Logger.DebugListLoaded("commandes achat", commandes.Count);
        }
        catch (Exception ex)
        {
            Logger.ErrorListLoadFailed(ex, "commandes achat");
        ...
        {
            _grid.Rows.Add(
                c.NumeroCommandeAchat,
                c.DateCommandeAchat.ToString("dd/MM/yyyy"),
                c.Fournisseur?.NomFournisseur,
    ...
    private void EditSelected()
    {
        ...
        if (numero is null)
        {
            MessageBox.Show(this, "Sélectionnez une commande à modifier.", "Commandes Achat", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
    ...
    {
        using var editor = new CommandeAchatEditForm(numero);
    ...
    private async Task CloneSelectedAsync()
    {
        ...
        if (numero is null)
        {
            MessageBox.Show(this, "Sélectionnez une commande à cloner.", "Commandes Achat", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        try
        {
            Logger.DebugCloning("commande achat", numero);
            var clone = await commandeService.CloneAsync(numero);
            Logger.DebugCloned("Commande achat", numero, clone.NumeroCommandeAchat);
            await LoadAsync();
            OpenEditor(clone.NumeroCommandeAchat);
        }
        catch (Exception ex)
        {
            Logger.ErrorCloneFailed(ex, "commande achat", numero);
    ...
    private async Task DeleteSelectedAsync()
    {
        ...
        if (numero is null)
        {
            MessageBox.Show(this, "Sélectionnez une commande à supprimer.", "Commandes Achat", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        try
        {
            Logger.DebugDeleting("commande achat", numero);
            await commandeService.DeleteAsync(numero);
            Logger.DebugDeleted("Commande achat", numero);
            await LoadAsync();
        }
        catch (Exception ex)
        {
            Logger.WarningDeleteFailed(ex, "commande achat", numero);
