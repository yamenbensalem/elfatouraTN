...
namespace GestCom_Desktop.Forms.BonsLivraison;
...
public class BonLivraisonEditForm : Form
{
    ...
    private async Task LoadAsync()
    {
        ...
        if (!_isNew)
        {
            ...
            if (bon is null)
            {
                Logger.WarningNotFound("Bon de livraison", _numero);
    ...
    private async Task SaveAsync()
    {
        ...
        var bon = new Web_GestCom.Data.Models.BonLivraison
        {
        ...
        try
        {
            Logger.DebugSaving("bon de livraison", _numero, _isNew, lignes.Count);
            using var scope = AppHost.CreateScope();
            var bonService = scope.ServiceProvider.GetRequiredService<IBonLivraisonService>();
            ...
            Logger.DebugSaved("Bon de livraison", bon.NumeroBonLivraison);
            DialogResult = DialogResult.OK;
            Close();
        }
        catch (Exception ex)
        {
            Logger.ErrorSaveFailed(ex, "bon de livraison", _numero);
    ...
    private async Task PrintAsync()
    {
        ...
        {
            Logger.ErrorPrintFailed(ex, "bon de livraison", _numero);
