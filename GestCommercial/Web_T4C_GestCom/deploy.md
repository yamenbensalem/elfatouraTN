...
```bash
dotnet publish Web_T4C_GestCom.csproj \
  -c Release \
  -r win-x64 \
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=MON_SERVEUR;Database=T4C_GestCom;User Id=SA;Password=***;TrustServerCertificate=True"
  },
  "AppConfig": {
    "TimbreFiscal": "0.6",
    "TauxRetenue": "1.5",
...
Server=192.168.1.10;Database=T4C_GestCom;User Id=t4c;Password=***;TrustServerCertificate=True
...
```bat
sc create T4CGestCom ^
  binPath= "C:\GestCom\publish\Web_T4C_GestCom.exe --urls http://0.0.0.0:5000" ^
  start= auto ^
  DisplayName= "T4C GestCom Web"
...
```bat
reg add "HKLM\SYSTEM\CurrentControlSet\Services\T4CGestCom\Parameters" ^
  /v "AppParameters" /t REG_SZ /d "--urls http://0.0.0.0:5000"
reg add "HKLM\SYSTEM\CurrentControlSet\Services\T4CGestCom\Parameters\AppSettings" ^
...
2. IIS Manager → **Add Website**
   - Physical path : `C:\inetpub\T4CGestCom\publish\`
   - Port : `80`
...
dotnet publish Web_T4C_GestCom.csproj -c Release -r win-x64 --self-contained false -o ./publish
...
| Migration échoue | Droits insuffisants | Donner `db_owner` au compte SQL sur `T4C_GestCom` |
