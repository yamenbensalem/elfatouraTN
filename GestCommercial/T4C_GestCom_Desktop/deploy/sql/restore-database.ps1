...
<#
.SYNOPSIS
    Restaure la base T4C_GestCom initiale (schéma + données de référence + admin par défaut) sur
...
.DESCRIPTION
    ...
    AVANT create-app-login.sql (qui a besoin que la base T4C_GestCom existe déjà pour y créer le
...
.PARAMETER BackupFile
    Chemin du .bak à restaurer. Par défaut : Database\T4C_GestCom_Template.bak à côté de ce script
    (c'est l'emplacement où build-client-package.ps1 le copie dans le package livré au client).

.PARAMETER TargetDatabaseName
    Nom de la base restaurée. Par défaut "T4C_GestCom" — c'est le nom attendu par
    T4C_GestCom_Desktop/appsettings.json (ConnectionStrings:DefaultConnection).
...
.EXAMPLE
    ...
    Restaure Database\T4C_GestCom_Template.bak vers la base "T4C_GestCom" sur l'instance locale.

.EXAMPLE
    ./restore-database.ps1 -BackupFile "C:\Temp\T4C_GestCom_Template.bak" -Server ".\SQLEXPRESS" -Force
#>
param(
    [string]$BackupFile = (Join-Path $PSScriptRoot "T4C_GestCom_Template.bak"),
    [string]$TargetDatabaseName = "T4C_GestCom",
    [string]$Server = ".",
    [switch]$Force
)
...
Write-Host "T4C_GestCom — restauration de la base initiale" -ForegroundColor Cyan
