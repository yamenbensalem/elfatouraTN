-- Run once on the CLIENT's SQL Server instance, as an admin (sysadmin / db_owner on T4C_GestCom).
--
-- Creates a dedicated, least-privilege Windows login for T4C_GestCom_Desktop instead of running
...
--   net user svc_t4cgestcom "<a strong random password>" /add
--   (local account; it never interactively logs in, so Windows won't force a password change)
--
-- Then point T4C_GestCom_Desktop/appsettings.json's DefaultConnection at this account by running
-- the app as that user (e.g. via a scheduled task configured to run as svc_t4cgestcom, or a
-- shortcut using `runas /user:.\svc_t4cgestcom`) — Trusted_Connection stays True, so no password
-- ever appears in appsettings.json.

DECLARE @MachineName sysname = CAST(SERVERPROPERTY('MachineName') AS sysname);
DECLARE @LoginName sysname = @MachineName + N'\svc_t4cgestcom';
DECLARE @Sql nvarchar(max);

IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE name = @LoginName)
BEGIN
    SET @Sql = N'CREATE LOGIN [' + @LoginName + N'] FROM WINDOWS WITH DEFAULT_DATABASE = [T4C_GestCom];';
    EXEC (@Sql);
END
...
PRINT N'Login ' + @LoginName + N' created and scoped to db_datareader/db_datawriter on T4C_GestCom.';
