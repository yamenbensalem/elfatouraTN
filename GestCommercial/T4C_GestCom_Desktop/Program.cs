...
{
    /// <summary>
    /// %ProgramData%\T4C_GestCom\logs by default — shared regardless of which Windows account runs
    ...
    static void Main()
    {
        ...
        try
        {
            Log.Information("Démarrage de T4C GestCom Desktop.");
    ...
    {
        var programDataDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "T4C_GestCom", "logs");
        try
        {
        ...
        {
            var fallbackDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "T4C_GestCom", "logs");
